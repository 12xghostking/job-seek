...
import { User, Building2, Mail, Lock, LogIn, ArrowRight } from 'lucide-react';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (role === 'user') {
        navigate(`/seeker?name=${encodeURIComponent(userData.name)}`);
      } else {
        navigate(`/employer?name=${encodeURIComponent(userData.name)}`);
      }
    } catch (err) {
      ...
      <div className="container flex-grow-1 d-flex align-items-center justify-content-center py-5">
        ...
        >
          <div className="text-center mb-4">
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '8px' }}>Sign in to JobSeek</h2>
            <p className="text-muted small">Select your role and enter your credentials to continue</p>
          </div>
          ...
          <div className="text-center mt-4 pt-3 border-top small text-muted">
            ...
            <Link to="/signup" className="fw-semibold text-decoration-none" style={{ color: 'var(--primary)' }}>
